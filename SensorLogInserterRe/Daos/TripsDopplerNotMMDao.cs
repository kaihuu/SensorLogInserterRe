...
{
    class TripsDopplerNotMMDao
    {
        private static readonly string TableName = "TRIPS_Doppler_NotMM";
        ...
        public static void DeleteTrips()
        {
            ...
            query.AppendLine($"FROM {EcologDopplerNotMMDao.TableName}");
            query.AppendLine($"WHERE {EcologDopplerNotMMDao.ColumnTripId} = {TripsDopplerNotMMDao.ColumnTripId}");
        ...
        public static DataTable Get(InsertDatum datum)
        {
            ...
            query.AppendLine("SELECT *");
            query.AppendLine($"FROM {TableName}");
            query.AppendLine($"WHERE {TripsDopplerNotMMDao.ColumnDriverId} = {datum.DriverId}");
            query.AppendLine($"AND {TripsDopplerNotMMDao.ColumnCarId} = {datum.CarId}");
            query.AppendLine($"AND {TripsDopplerNotMMDao.ColumnSensorId} = {datum.SensorId}");
            query.AppendLine($"AND {TripsDopplerNotMMDao.ColumnStartTime} >= '{datum.StartTime}'");
            query.AppendLine($"AND {TripsDopplerNotMMDao.ColumnEndTime} <= '{datum.EndTime}'");
        ...
        public static void UpdateConsumedEnergy()
        {
            ...
            selectQuery.AppendLine($"FROM {TableName} AS trip, {EcologDopplerNotMMDao.TableName} AS ecolog");
